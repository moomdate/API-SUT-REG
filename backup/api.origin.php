...
foreach ($data3  as $key => $value) {
	...
	for($i=0;$i<7;$i++){ //get date
		...
		if($map_==1){ //ถ้าเป็นวันที่เรียน map_ จะเท่ากับ 1++
			//$dataIngroups[$countTime] = "xxx";


			if(strpos($value,$data_dot)>30){ //detech found in date

			}else{


			if($befor == $counter){ // first group 
				$dataIngroups[$countTime]['Date'] = $Date_[$i]; //ad date to array
				echo "<br>เรียนวัน ".$Date_[$i];
				echo "<br>";
			//echo  strlen($Date_[$i]);
				echo "เวลา:";
				echo $time__ =  substr($value,(strlen($Date_[$i])),11);
				...
				echo "<br>";
				$howlon = 0;
				for($gg = strlen($detalNonesub)-2;$gg>=0;$gg--){ //check tyoe
				...
				$howlon2 = 0;
				for($lop=$howlon-1;$lop>=0;$lop--){ //check building
				...
				echo "ห้อง:";
				echo $room = substr($detalNonesub,0,$howlon2+2);
			...
			//echo  strlen($Date_[$i]);
				...
				echo "เวลา:";
				echo $time_2 =  substr($value,strlen($Date_[$i])+8,11); //(strlen($Date_[$i]+20))
				...
				echo "ห้อง:";
				echo $room = substr($detalNonesub,0,$howlon2);
				$dataIngroups[$countTime]['Room']  =   $room; // add room to array
				echo "<br>";
				//echo $howlon2;
				echo "อาคาร:";
